...
export default function RevenueChart({ byDay, byMonth, currentMonthByDay, prevMonthByDay }: RevenueChartProps) {
  ...
  const comparisonData = useMemo(() => {
    ...
    return allDays.map(d => {
      ...
      return {
        day: d,
        [`cur_${metric}`]:  cur  ? cur[metric as keyof typeof cur]  : null,
        [`prev_${metric}`]: prev ? prev[metric as keyof typeof prev] : null,
      };
    });
  ...
  return (
    <div className="bg-brand-card border border-brand-border rounded-2xl p-4 sm:p-6">
      <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
        <div>
          ...
          <p className="text-brand-sub text-sm mt-0.5">
            {view === "day" ? "Día a día · mes anterior completo en punteado" : "Ingresos acumulados por mes"}
          </p>
        </div>
          ...
          <div className="flex bg-brand-dark border border-brand-border rounded-lg overflow-hidden">
            {(["revenue", "margen", "orders"] as const).map(m => (
              <button key={m} onClick={() => setMetric(m)}
                className={`px-2.5 py-1.5 text-xs font-mono transition-all ${metric === m ? "bg-brand-yellow text-brand-dark font-bold" : "text-brand-sub hover:text-brand-text"}`}>
          ...
          <div className="flex bg-brand-dark border border-brand-border rounded-lg overflow-hidden">
            {(["day", "month"] as const).map(v => (
              <button key={v} onClick={() => setView(v)}
                className={`px-2.5 py-1.5 text-xs font-mono transition-all ${view === v ? "bg-brand-yellow text-brand-dark font-bold" : "text-brand-sub hover:text-brand-text"}`}>
      ...
      <ResponsiveContainer width="100%" height={240}>
        {view === "month" ? (
          <BarChart data={monthData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1E1E2E" vertical={false} />
            <XAxis dataKey="label" tickFormatter={formatMonth}
              tick={{ fill: "#8888AA", fontSize: 10, fontFamily: "DM Mono" }} axisLine={false} tickLine={false} />
            <YAxis tickFormatter={metric !== "orders" ? formatCurrency : undefined}
              tick={{ fill: "#8888AA", fontSize: 10, fontFamily: "DM Mono" }} axisLine={false} tickLine={false} width={50} />
          ...
          <AreaChart data={comparisonData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
            ...
            <CartesianGrid strokeDasharray="3 3" stroke="#1E1E2E" vertical={false} />
            <XAxis dataKey="day" tick={{ fill: "#8888AA", fontSize: 10, fontFamily: "DM Mono" }} axisLine={false} tickLine={false} />
            <YAxis tickFormatter={metric !== "orders" ? formatCurrency : undefined}
              tick={{ fill: "#8888AA", fontSize: 10, fontFamily: "DM Mono" }} axisLine={false} tickLine={false} width={50} />
            <Tooltip content={<ComparisonTooltip metric={metric} />} cursor={{ stroke: "#333355" }} />
